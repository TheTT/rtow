...
int main(){
  ...
  std::shared_ptr<Material> m4=std::make_shared<Metal>(Col(.8,.7,.8),0.05);
  // std::shared_ptr<Material> m2=std::make_shared<Nuclear>(1.8);
  std::shared_ptr<Material> m3=std::make_shared<Metal>(Col(.6,.6,.6),0.05);
#ifdef RENDER
...
#endif
  auto cam=Camera()
    .setDire(Point(0,0,2.5),Point(0,0,1.5),Vec3d(0,1,0))
    .setRes(w,h,esr)
    .setFov(d2r(60));
  auto trc=Tracer()
    .setCam(
